...
7. **If it is not a defect, it does not go in the review.** There is no suggestions section — no "consider…", no optional improvements, no preferences. "Non-blocking" is not free: the author still pays a context switch to read it, decide whether you meant it, and answer it, and the ones that are wrong cost the entire round trip they were supposedly too cheap to matter. A review that mixes defects with taste teaches the author to skim, and what gets skimmed is the blocker.
